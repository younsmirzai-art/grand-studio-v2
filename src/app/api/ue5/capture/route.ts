...
const UPGRADE_MSG = "You've reached your daily screenshot limit. Upgrade to Pro for unlimited screenshots!";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const limitCheck = await checkUsageLimit(userId, "screenshot");
    if (!limitCheck.allowed) {
      return NextResponse.json(
        { error: UPGRADE_MSG, limitReached: true },
        { status: 403 }
      );
    }
    ...
    await recordUsage(userId, "screenshot");
